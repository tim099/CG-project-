...
void Image::convert_to_sobel(Image* image,glm::vec2 dv){
	...
	for(int i=0;i<image->size.y;i++){
		for(int j=0;j<image->size.x;j++){
			...
			GX=-dv.y*arr[0][0]+dv.y*arr[0][2]-dv.x*arr[1][0]+dv.x*arr[1][2]-dv.y*arr[2][0]+dv.y*arr[2][2];
			GY=-dv.y*arr[0][0]+dv.y*arr[2][0]-dv.x*arr[0][1]+dv.x*arr[2][1]-dv.y*arr[0][2]+dv.y*arr[2][2];
